...
(function () {
    ...
    var isStrict = document.compatMode == "CSS1Compat", isOpera = ua.indexOf("opera") > -1,
        isSafari = (/webkit|khtml/).test(ua), isSafari3 = isSafari && ua.indexOf("webkit/5") != -1,
        isIE = !isOpera && ua.indexOf("msie") > -1, isIE7 = !isOpera && ua.indexOf("msie 7") > -1,
        isGecko = !isSafari && ua.indexOf("gecko") > -1, isGecko3 = !isSafari && ua.indexOf("rv:1.9") > -1,
        isBorderBox = isIE && !isStrict, isWindows = (ua.indexOf("windows") != -1 || ua.indexOf("win32") != -1),
        isMac = (ua.indexOf("macintosh") != -1 || ua.indexOf("mac os x") != -1), isAir = (ua.indexOf("adobeair") != -1),
        isLinux = (ua.indexOf("linux") != -1), isSecure = window.location.href.toLowerCase().indexOf("https") === 0;
    if (isIE && !isIE7) {
        try {
...
(function () {
    ...
    El.prototype = {
        originalDisplay: "", visibilityMode: 1, defaultUnit: "px", setVisibilityMode: function (visMode) {
            ...
            var el = this.dom;
            var o = this.getOffsetsTo(c), l = o[0] + c.scrollLeft, t = o[1] + c.scrollTop, b = t + el.offsetHeight,
                r = l + el.offsetWidth;
...
Ext.extend(Ext.data.Connection, Ext.util.Observable, {
    timeout: 30000, autoAbort: false, disableCaching: true, disableCachingParam: "_dc", request: function (E) {
...
Ext.extend(Ext.util.ClickRepeater, Ext.util.Observable, {
    interval: 20, delay: 250, preventDefault: true, stopDefault: false, timer: 0, handleMouseDown: function () {
...
Ext.extend(Ext.dd.DragTracker, Ext.util.Observable, {
    active: false, tolerance: 5, autoStart: false, initEl: function (A) {
        this.el = Ext.get(A);
...
Ext.dd.StatusProxy.prototype = {
    dropAllowed: "x-dd-drop-ok", dropNotAllowed: "x-dd-drop-nodrop", setStatus: function (A) {
...
Ext.extend(Ext.dd.DragSource, Ext.dd.DDProxy, {
    dropAllowed: "x-dd-drop-ok", dropNotAllowed: "x-dd-drop-nodrop", getDragData: function (A) {
...
Ext.extend(Ext.data.Store, Ext.util.Observable, {
    remoteSort: false, pruneModifiedRecords: false, lastOptions: null, destroy: function () {
        if (this.id) {
...
Ext.data.JsonStore = function (A) {
    Ext.data.JsonStore.superclass.constructor.call(this, Ext.apply(A, {
        proxy: A.proxy || (!A.data ? new Ext.data.HttpProxy({url: A.url}) : undefined),
...
Ext.extend(Ext.data.ScriptTagProxy, Ext.data.DataProxy, {
    timeout: 30000, callbackParam: "callback", nocache: true, load: function (E, F, H, I, J) {
        if (this.fireEvent("beforeload", this, E) !== false) {
...
Ext.data.GroupingStore = Ext.extend(Ext.data.Store, {
    remoteGroup: false, groupOnSort: false, clearGrouping: function () {
...
Ext.layout.AnchorLayout = Ext.extend(Ext.layout.ContainerLayout, {
    monitorResize: true, getAnchorViewSize: function (A, B) {
...
Ext.layout.BorderLayout = Ext.extend(Ext.layout.ContainerLayout, {
    monitorResize: true, rendered: false, onLayout: function (B, X) {
        var C;
        if (!this.rendered) {
            X.position();
...
Ext.form.CheckboxGroup = Ext.extend(Ext.form.Field, {
    ...
    onRender: function (H, F) {
        if (!this.el) {
            var M = {cls: this.groupCls, layout: "column", border: false, renderTo: H};
            var A = {
            ...
            if (this.items[0].items) {
                Ext.apply(M, {layoutConfig: {columns: this.items.length}, defaults: this.defaults, items: this.items});
                ...
                Ext.apply(M, {layoutConfig: {columns: D}, items: K})
            }
...
Ext.extend(Ext.form.BasicForm, Ext.util.Observable, {
    timeout: 30, activeAction: null, trackResetOnLoad: false, initEl: function (A) {
        this.el = Ext.get(A);
...
Ext.extend(Ext.grid.HeaderDropZone, Ext.dd.DropZone, {
    proxyOffsets: [-4, -9], fly: Ext.Element.fly, getTargetFromEvent: function (C) {
        var A = Ext.lib.Event.getTarget(C);
...
Ext.StatusBar = Ext.extend(Ext.Toolbar, {
    ...
    setStatus: function (C) {
        ...
        if (C.clear) {
            var D = C.clear, B = this.autoClear, A = {useDefaults: true, anim: true};
            if (typeof D == "object") {
                D = Ext.applyIf(D, A);
                if (D.wait) {
                    B = D.wait
    ...
    clearStatus: function (C) {
        ...
        if (C.anim) {
            this.statusEl.fadeOut({
                remove: false, useDisplay: true, scope: this, callback: function () {
...
Ext.History = (function () {
    ...
    return {
        fieldId: "x-history-field", iframeId: "x-history-frame", events: {}, init: function (L, K) {
            if (J) {
...
Ext.ux.Clipboard = function () {
    return {
        set: function (a) {
            if (window.clipboardData) window.clipboardData.setData("Text", String(a)); else if (window.netscape) try {
            ...
            } else return alert("Your browser does not support this feature")
        }, get: function () {
            if (window.clipboardData) return window.clipboardData.getData("Text");
            else if (window.netscape) try {
                netscape.security.PrivilegeManager.enablePrivilege("UniversalXPConnect");
                var a = Components.classes["@mozilla.org/widget/clipboard;1"].getService(Components.interfaces.nsIClipboard);
                if (!a) return false;
                ...
                a.getData(b, a.kGlobalClipboard);
                a = {};
                var c = {};
                try {
                    b.getTransferData("text/unicode", a, c)
                } catch (e) {
                    return
                }
                b =
                    "";
                if (a) a = a.value.QueryInterface(Components.interfaces.nsISupportsString);
                if (a) b = a.data.substring(0, c.value / 2);
                return b
            } catch (d) {
                return alert(d + '\n\nPlease type: "about:config" in your address bar.\nThen filter by "signed".\nChange the value of "signed.applets.codebase_principal_support" to true.\nYou should then be able to use this feature.')
            } else return alert("Your browser does not support this feature")
    ...
    return {
        get: function (c) {
            ...
            if (typeof a[c] === "undefined") {
                a[c] = "icon_" + Ext.id();
                var d = "\n." + a[c] + " { background-image: url(" + c + ") !important; }";
                Ext.isIE ? document.styleSheets.styleSheetIconCls.cssText += d : Ext.get("styleSheetIconCls").dom.sheet.insertRule(d, 0)
            }
            return a[c]
        }
    }
...
String.space = function (a) {
    var c = [], d;
    for (d = 0; d < a; d++) c.push(" ");
    return c.join("")
};

function aboutWindow() {
    var a = [];
    Ext.getBody().select("div.tab").each(function (c) {
        a.push({
            title: c.select("h2").first().dom.innerHTML,
            html: c.select("div").first().dom.innerHTML.replace("{gabor}", '<a href="mailto:turi.gabor@gmail.com">Gabor Turi</a>')
        })
    });
    (new Ext.Window({
...
Ext.onReady(function () {
                ...
                i = new Ext.Window({
                    ...
                    buttons: [{
                        text: "Load JSON data!", handler: function () {
                            f.loadJson(i.items.get(0).getValue());
                            i.hide()
                        }
                    }],
                    listeners: {
                        resize: function (i, a) {
                            i.items.get(0).setWidth(a -
                                50)
                        }
                    }
                })
    ...
    var d = {
        ...
        d = {
            id: "viewerPanel", layout: "border", title: "Viewer", region:"center", items: [{
                id: "tree",
                xtype: "treepanel",
                region: "center",
                ...
                listeners: {
                    render: function (i) {
                        i.getSelectionModel().on("selectionchange", function (i, a) {
                            f.gridbuild(a)
                        })
                    }, contextmenu: function (a, c) {
                        (new Ext.menu.Menu({
                            items: [{
    ...
    new Ext.Viewport({
        layout: "border", items: [d, {
            ...
            id: "textPaneal", layout: "fit", title: "Texzt", region:"east", width:0, items: [{
                id: "edit",
                xtype: "textarea", style: "font-family:monospace;display:none", emptyText: "Copy here the JSON variable!"
            }]
        }/*{
            xtype: "tabpanel", items: [d, {
                id: "textPanel", layout: "fit", title: "Text", /!*tbar: [{
                    ...
                    id: "cho",
                    xtype: "button", style: "width:100px",
                }, {
                    id: "edit",
                    xtype: "textarea", style: "font-family:monospace;display:none", emptyText: "Copy here the JSON variable!"
                }]
            }], activeTab: "textPanel", listeners: {
                beforetabchange: function (a, c) {
                    if (c.id === "viewerPanel") return f.check()
                }, tabchange: function (a, f) {
                    f.id === "viewerPanel" ? c.enable() : c.disable()
                }
            }
        }*/]
    });
    var f = function () {
        var a = Ext.getCmp("edit"), c = Ext.getCmp("tree"), d = c.getRootNode(), p = Ext.getCmp("grid"),
            k = Ext.getCmp("searchTextField"), n = Ext.getCmp("searchResultLabel"), l = {}, o = null, m = null,
            g = null, j = null;
        return {
            check: function () {
                var e =
                ...
                o !== e && (o = e, this.treebuild())
            }, treebuild: function () {
                d.removeAllChildren();
                d.appendChild(this.json2leaf(l));
                d.setIcon(Ext.isArray(l) ? "array.gif" : "object.gif");
                this.gridbuild(d);
                d.expand.defer(50, d, [!1, !1])
            }, gridbuild: function (e) {
                if (e.isLeaf()) e = e.parentNode;
                e.childNodes.length ||
                (e.expand(!1, !1), e.collapse(!1, !1));
                for (var a = {}, b = 0; b < e.childNodes.length; b++) {
                    var c = e.childNodes[b].text.indexOf(":");
                    c === -1 ? a[e.childNodes[b].text] = "..." : a[e.childNodes[b].text.substring(0, c)] = e.childNodes[b].text.substring(c + 1)
                }
                p.setSource(a)
            }, json2leaf: function (e) {
                var a = [], b;
                for (b in e) e.hasOwnProperty(b) && (e[b] === null ? a.push({
                    text: b + " : null",
                    leaf: !0,
                    icon: "red.gif"
                }) : typeof e[b] === "string" ? a.push({
                    text: b + ' : "' + e[b] + '"',
                    leaf: !0,
                    icon: "blue.gif"
                }) : typeof e[b] === "number" ? a.push({
                    text: b + " : " + e[b],
                    leaf: !0, icon: "green.gif"
                }) : typeof e[b] === "boolean" ? a.push({
                    text: b + " : " + (e[b] ? "true" : "false"),
                    leaf: !0,
                    icon: "yellow.gif"
                }) : typeof e[b] === "object" ? a.push({
                    text: b,
                    children: this.json2leaf(e[b]),
                    icon: Ext.isArray(e[b]) ? "array.gif" : "object.gif"
                }) : typeof e[b] === "function" && a.push({text: b + " : function", leaf: !0, icon: "red.gif"}));
                return a
            }, copyText: function () {
                ...
                g = [];
                k.getValue() && (this.searchInNode(d, k.getValue()), g.length ? (n.setText(""), j = 0, this.selectNode(g[j]), k.focus()) : n.setText("Phrase not found!"))
            }, searchInNode: function (a, c) {
                a.text.toUpperCase().indexOf(c.toUpperCase()) !== -1 && g.push(a);
                var b = a.isExpanded();
                a.expand(!1, !1);
                for (var d = 0; d < a.childNodes.length; d++) this.searchInNode(a.childNodes[d], c);
                b || a.collapse(!1, !1)
            }, selectNode: function (a) {
                a.select();
                for (c.fireEvent("click", a); a !== d;) a = a.parentNode, a.expand(!1, !1)
            }, searchNext: function () {
                g && g.length && (j = (j + 1) % g.length, this.selectNode(g[j]))
            }, searchPrevious: function () {
                g && g.length && (j = (j - 1 + g.length) % g.length, this.selectNode(g[j]))
            }, ctrlF: function () {
                c.getBottomToolbar().isVisible() || c.showBbar();
                k.focus(!0)
            }, hideToolbar: function () {
                c.hideBbar()
            }, format: function () {
                for (var e = a.getValue().replace(/\n/g, " ").replace(/\r/g, " "), c = [], b = 0, d = !1, f = 0, g = e.length; f < g; f++) {
                    var h = e.charAt(f);
                    if (d && h === d) e.charAt(f -
                        1) !== "\\" && (d = !1); else if (!d && (h === '"' || h === "'")) d = h; else if (!d && (h === " " || h === "\t")) h = ""; else if (!d && h === ":") h += " "; else if (!d && h === ",") h += "\n" + String.space(b * 2); else if (!d && (h === "[" || h === "{")) b++, h += "\n" + String.space(b * 2); else if (!d && (h === "]" || h === "}")) b--, h = "\n" + String.space(b * 2) + h;
                    c.push(h)
                }
                a.setValue(c.join(""))
            }, removeWhiteSpace: function () {
                for (var e = a.getValue().replace(/\n/g, " ").replace(/\r/g, " "), c = [], b = !1, d = 0, f = e.length; d < f; d++) {
                    var g = e.charAt(d);
                    if (b && g === b) e.charAt(d - 1) !== "\\" && (b = !1);
                    else if (!b && (g === '"' || g === "'")) b = g; else if (!b && (g === " " || g === "\t")) g = "";
                    c.push(g)
                }
                a.setValue(c.join(""))
            }, loadJson: function (a) {
    ...
    document.location.hash.length && f.loadJson(document.location.hash.substring(1))
});